...
object CreateMetadataActionFactory {
  def getCreateMetadataAction(config: GlobalConfig, conf: Map[String, String], source: Node, target: Node) : CreateMetadataAction = {
    def targetType = GeneralUtilities.getConfStrict(conf, GeneralConstants.targetTypeProp, "Internally-provided")
    targetType match {
      case (GeneralConstants.awsType) => new CreateMetadataHiveAction(config, conf, target)
      case (GeneralConstants.localClusterType) => new CreateMetadataHiveAction(config, conf, target)
